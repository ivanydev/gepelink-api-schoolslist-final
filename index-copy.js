...
async function generateCSV() {
  ...
  for (const [table, uid] of Object.entries(FORMS)) {
    ...
    submissions.forEach((row, index) => {
      ...
      // Ajustar nomes de campos conforme o XLSForm
      const codigo = row["identificacao_da_escola/DGE_SQE_B0_P1_codigo_escola"];
      const nome = row["identificacao_da_escola/DGE_SQE_B0_P1_nome_escola"];
      const provincia = row["identificacao_da_escola/DGE_SQE_B0_P1_provincia"];
      const municipio = row["identificacao_da_escola/DGE_SQE_B0_P1_municipio"];
      const comuna = row["identificacao_da_escola/DGE_SQE_B0_P1_comuna"];
  ...
  // Gerar CSV final
  const linhas = ["DGE_SQE_B0_P1_codigo_escola,DGE_SQE_B0_P1_nome_escola,DGE_SQE_B0_P1_provincia,DGE_SQE_B0_P1_municipio,DGE_SQE_B0_P1_comuna"];
